...
fn word_search(needle: &[char], haystack: &[Vec<char>], coord: (isize, isize), dir: (isize, isize), curr: &mut Vec<char>) -> bool {
    if curr == needle {
        return true;
    }
    ...
    return word_search(needle, haystack, (coord.0+dir.0, coord.1+dir.1), dir, curr);
}
    ...
    for i in 0..grid.len() {
        for j in 0..grid[i].len() {
            for dir in dirs.iter() {
                if word_search(&['X', 'M', 'A', 'S'], &grid, (i as isize, j as isize), *dir, &mut Vec::new()) {
                    tot += 1;
                }
            } 
        }
